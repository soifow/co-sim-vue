  ...
  if (config.method.toLocaleLowerCase() == "post" || config.method.toLocaleLowerCase() == "put") {
    ...
    if (config.data&& typeof config.data!= "string") {
      if (config.data.id) {
        ...
        config.data.createBy = userInfo.id
      }
    }
  }
  //--添加createBy、updateBy end--
  ...
  fileReader.onload = function () {
    try {
      ...
      if (jsonData.status === 500) {
        ...
        if (token &&jsonData.message&& jsonData.message.includes("Token失效")) {
          Modal.error({
            title: '登录已过期',
            content: '很抱歉，登录已过期，请重新登录',
            okText: '重新登录',
...
const uploadFile = function (url, parameter) {
  const token = Vue.ls.get(ACCESS_TOKEN)
